...
class StreamDeckManager(ctk.CTk):
    ...
    def create_layout(self):
        ...
        self.quick_actions = [
            {"name": "Discord Mute", "icon": "🎤", "hotkey": "ctrl+shift+m"},
            {"name": "Discord Deafen", "icon": "🔇", "hotkey": "ctrl+shift+d"},
            {"name": "OBS Start/Stop", "icon": "⏺️", "hotkey": "ctrl+shift+r"},
            {"name": "OBS Switch Scene", "icon": "🎬", "hotkey": "ctrl+shift+s"},
            {"name": "Spotify Play/Pause", "icon": "⏯️", "hotkey": "ctrl+shift+p"},
            {"name": "Volume Up", "icon": "🔊", "hotkey": "volumeup"},
            {"name": "Volume Down", "icon": "🔉", "hotkey": "volumedown"},
            {"name": "Mute Audio", "icon": "🔇", "hotkey": "volumemute"},
            {"name": "Screenshot", "icon": "📸", "hotkey": "win+shift+s"},
            {"name": "Task Manager", "icon": "⚙️", "hotkey": "ctrl+shift+esc"},
        ]
    ...
    def create_slider_widget(self, parent, slider_index):
        ...
        header = ctk.CTkLabel(
            slider_frame,
            text=f"🎚️ Slider {slider_index + 1}",
            font=("Roboto", 16, "bold")
        )
    ...
    def configure_button(self, btn_index):
        ...
        ctk.CTkLabel(
            header_frame,
            text=f"Button {btn_index + 1} - Mode {self.current_mode + 1}",
            font=("Roboto", 24, "bold")
        ).pack(pady=20)
...
• Arrows: up, down, left, right"""
        ...
        def save_config():
            ...
            if not hotkey or not key:
                error_label = ctk.CTkLabel(
                    content,
                    text="❌ Main key mag niet leeg zijn!",
                    font=("Roboto", 12, "bold"),
                    text_color="red"
                )
    ...
    def update_button_display(self, btn_index):
        ...
        if config:
            btn['icon'].configure(text=config.get('icon', '🎮'))
    ...
    def connect_serial(self):
        ...
        if not ports:
            self.info_label.configure(text="❌ Geen devices gevonden\n\nSluit je Arduino/Pico aan")
            return
        ...
        def connect():
            try:
                ...
                self.serial_button.configure(text="✅ Connected", fg_color="green")
                self.serial_status.configure(text="✅ Connected", fg_color="green")
                self.info_label.configure(text=f"✅ Connected to:\n{port_var.get()}\n\nSyncing config...")
                dialog.destroy()
    ...
    def send_to_device(self, message):
        ...
        if self.serial_port and self.serial_port.is_open:
            try:
                ...
                self.serial_status.configure(text="❌ Disconnected", fg_color="red")
    ...
    def send_all_configs(self):
        ...
        self.info_label.configure(text=f"✅ Sync complete!\n\n{count} buttons configured\n3 sliders configured")
        print(f"✅ Sent {count} button configs + 3 sliders to device")
    ...
    def export_config(self):
        ...
        if filename:
            ...
            self.info_label.configure(text=f"✅ Exported to:\n{Path(filename).name}")
    ...
    def import_config(self):
        ...
        if filename:
            ...
            self.info_label.configure(text=f"✅ Imported from:\n{Path(filename).name}")
